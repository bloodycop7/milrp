    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget then
			...
			if plyTarget.InJail then
				mrp.Arrest.Prison[plyTarget.InJail][plyTarget:EntIndex()] = nil
				plyTarget.InJail = nil
				timer.Remove(plyTarget:UserID().."mrpPrison")
				plyTarget:StopDrag()
				plyTarget:Spawn()
			end
		else
...
mrp.RegisterChatCommand("/unarrest", unArrestCommand)
...
local setTeamCommand = {
    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget then
			if teamID and mrp.Teams.Data[teamID] then
...
mrp.RegisterChatCommand("/setteam", setTeamCommand)
...
mrp.RegisterChatCommand("/forceunlock", forceUnlockCommand)
...
mrp.RegisterChatCommand("/forcelock", forceLockCommand)
...
mrp.RegisterChatCommand("/removebuggeddoor", removeDoorCommand)
...
mrp.RegisterChatCommand("/forceselldoor", sellDoorCommand)