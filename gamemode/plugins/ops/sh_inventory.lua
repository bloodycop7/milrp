if SERVER then
	util.AddNetworkString("mrpOpsViewInv")
	util.AddNetworkString("mrpOpsRemoveInv")

	net.Receive("mrpOpsRemoveInv", function(len, ply)
	...
	net.Receive("mrpOpsViewInv", function()
		...
		for i=1,invSize do
			...
			local itemid = net.ReadUInt(16)
			local item = mrp.Inventory.Items[itemnetid]
			
			table.insert(invCompiled, {item, itemrestricted, itemequipped, itemid})
		end

		local searchMenu = vgui.Create("mrpSearchMenuAdmin")
...
mrp.RegisterChatCommand("/viewinv", viewInvCommand)
			...
			if plyTarget.InventoryRestorePoint then
                ...
                for v,k in pairs(player.GetAll()) do
                    if k:IsLeadAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:Nick().." restored "..plyTarget:Nick().."'s inventory.")
                    end
                end
			else
...
mrp.RegisterChatCommand("/restoreinv", restoreInvCommand)