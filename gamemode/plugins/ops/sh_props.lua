local cleanupCommand = {
    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget then
			mrp.Ops.CleanupPlayer(plyTarget)

			plyTarget:Notify("Your props have been removed by a game moderator ("..ply:Nick()..").")
...
mrp.RegisterChatCommand("/cleanup", cleanupCommand)
...
local cleanupAllCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        if countdown and tonumber(countdown) and tonumber(countdown) > 0 then
        	...
        	DOING_CLEANUP = true
        	timer.Create("mrpOpsCleanupClock", 10, r, function()
        		left = left - 10
...
mrp.RegisterChatCommand("/cleanupall", cleanupAllCommand)
...
mrp.RegisterChatCommand("/cleardecals", clearDecalsCommand)