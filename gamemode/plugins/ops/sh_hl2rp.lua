...
local combineBanCommand = {
    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget and plyTarget.mrpData then
			local curT = os.time()
			local endT = curT + time

			plyTarget.mrpData.CombineBan = endT
			plyTarget:SaveData()

			if plyTarget:IsCP() then
				plyTarget:SetTeam(mrp.Config.DefaultTeam)
			end

			local howLong = string.NiceTime(time)

			ply:Notify("You have combine banned "..plyTarget:Nick().." for "..howLong..".")
			plyTarget:Notify("You have been banned from the combine faction for "..howLong.." by a game moderator ("..ply:Nick()..").")
...
mrp.RegisterChatCommand("/combineban", combineBanCommand)
...
local combineUnBanCommand = {
    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget and plyTarget.mrpData then
			plyTarget.mrpData.CombineBan = nil
			plyTarget:SaveData()
...
mrp.RegisterChatCommand("/uncombineban", combineUnBanCommand)