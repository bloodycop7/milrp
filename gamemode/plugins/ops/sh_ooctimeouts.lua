...
mrp.OOCTimeouts = mrp.OOCTimeouts or {}
...
local timeoutCommand = {
    ...
    onRun = function(ply, arg, rawText)
		...
		if plyTarget then
			...
			mrp.OOCTimeouts[sid] = CurTime() + time
			...
			timer.Create("mrpOOCTimeout"..sid, time, 1, function()
				if not mrp.OOCTimeouts[sid] then return end

				mrp.OOCTimeouts[sid] = nil
			...
			ply:Notify("You have issued "..plyTarget:Nick().." an OOC timeout for "..t.." minutes.")

			for v,k in pairs(player.GetAll()) do
				k:AddChatText(infoCol, plyTarget:Nick().." has been given an OOC timeout for "..t.." minutes by a game moderator.")
			end
...
mrp.RegisterChatCommand("/ooctimeout", timeoutCommand)
...
local unTimeoutCommand = {
	...
	onRun = function(ply, arg, rawText)
		...
		if plyTarget then
			mrp.OOCTimeouts[plyTarget:SteamID()] = nil
...
mrp.RegisterChatCommand("/unooctimeout", unTimeoutCommand)