if CLIENT then
    net.Receive("mrpOpsItemSpawner", function()
        ...
        for v,k in pairs(player.GetAll()) do
            targetBox:AddChoice("PLAYER: "..k:Nick().." ("..k:Nick()..")", k:SteamID())
        end
        ...
        function targetBox:OnSelect(index, text, sid)
            ...
            elseif text == "Custom SteamID (Offline User)" then
                Derma_StringRequest("mrp", "Enter the SteamID (not 64 format) below:", "", function(sid)
        ...
        for v,k in pairs(mrp.Inventory.Items) do
    ...
    util.AddNetworkString("mrpOpsItemSpawner")
end
...
local giveItemCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        local query = mysql:Select("mrp_players")
        query:Select("id")
        ...
        query:Callback(function(result)
            ...
            if not mrp.Inventory.ItemsRef[item] then
            ...
            local mrpID = result[1].id

            mrp.Inventory.DBAddItem(mrpID, item)
...
mrp.RegisterChatCommand("/giveitem", giveItemCommand)
...
local itemSpawnerCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        net.Start("mrpOpsItemSpawner")
        net.Send(ply)
    end
}

mrp.RegisterChatCommand("/itemspawner", itemSpawnerCommand)