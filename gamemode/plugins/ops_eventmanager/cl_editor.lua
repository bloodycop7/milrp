function mrp.Ops.EventManager.SequenceLoad(path)
	...
	mrp.Ops.EventManager.Sequences[json.Name] = json

	return true
end

function mrp.Ops.EventManager.SequenceSave(name)
	local sequence = mrp.Ops.EventManager.Sequences[name]
	file.Write("mrp/ops/eventmanager/"..sequence.FileName..".json", util.TableToJSON(sequence, true))
end

function mrp.Ops.EventManager.SequencePush(name)
	local sequence = mrp.Ops.EventManager.Sequences[name]
	...
	net.Start("mrpOpsEMPushSequence")
...
function mrp.Ops.EventManager.GetVersionHash()
	return util.CRC(util.TableToJSON(mrp.Ops.EventManager.Config.Events))
end