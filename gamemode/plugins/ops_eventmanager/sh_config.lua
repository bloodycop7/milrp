...
local stringVars = {
	...
	["#PlayerNick"] = function() return LocalPlayer():Nick() end
}
...
mrp.Ops.EventManager.Config.CategoryIcons = {
...
mrp.Ops.EventManager.Config.TagColours = {
...
mrp.Ops.EventManager.Config.Events = {
	["empty"] = {
		Cat = "hidden",
		Prop = {},
		NeedUID = false,
	...
	["explode_cinematic"] = {
		...
		Clientside = false,
		Do = function(prop, uid)
			local tempEnt = ents.Create("mrp_usable")
			...
			timer.Simple(0.05, function()
				if IsValid(tempEnt) then
					mrp.MakeBigExplosion(tempEnt, prop["magnitude"], prop["debris"])
				end
			end)
		end
	},
	...
	["text"] = {
		...
		Clientside = true,
		Do = function(prop, uid)
			local text = vgui.Create("mrpFadeText")
	...
	["textarray"] = {
		...
		Do = function(prop, uid)
			...
			timer.Create("mrpOpsEMTextArray"..math.random(1,100000), prop["array_delay"], max, function()
				local text = vgui.Create("mrpFadeText")
	...
	["itemspawn"] = {
		...
		Clientside = false,
		Do = function(prop, uid)
			local x = mrp.Inventory.SpawnItem(prop["itemclass"], prop["pos"])
	...
	["ammobox_spawn"] = {
		...
		Do = function(prop, uid)
			...
			local ent = ents.Create("mrp_hl2rp_eventammobox")
			ent:SetPos(prop["pos"])
			ent:SetAngles(Angle(prop["ang"].x, prop["ang"].y, prop["ang"].z))
			ent:Spawn()
			ent:Activate()
	...
	["setcookie"] = {
		...
		Clientside = true,
		Do = function(prop, uid)
			cookie.Set("mrp_em_"..prop["name"], prop["value"])
		end
	},
	["npc_spawn"] = {
		Cat = "npc",
	...
	["citycodeset"] = {
		...
		Clientside = false,
		Do = function(prop, uid)
			mrp.Dispatch.SetCityCode(prop["code"])
			mrp.Dispatch.SetupCityCode(prop["code"])
		end
	},
	["voiceallowedset"] = {
		Cat = "server",
	...
	["createscene"] = {
		...
		Clientside = true,
		Do = function(prop, uid)
			if not mrp.Ops.EventManager.Scenes then
				return
			end
			...
			mrp.Ops.EventManager.Scenes[uid] = {prop}
		end
	},
	["playscene"] = {
		Cat = "scene",
	...
	["combinewaypoint"] = {
		...
		Do = function(prop, uid)
			if LocalPlayer():IsCP() then
				mrp.AddCombineWaypoint(prop["message"], Vector(prop["pos"].x, prop["pos"].y, prop["pos"].z), prop["duration"], prop["iconid"], prop["colid"], prop["textcolid"])
			end
		end
	},
	["combinemessage"] = {
		...
		Do = function(prop, uid)
			if LocalPlayer():IsCP() then
				mrp.AddCombineMessage(prop["message"], prop["col"], prop["nosound"])
			end
		end
	},
	["combineobjective"] = {
		...
		Do = function(prop, uid)
			...
			net.Start("mrpHL2RPObjectiveSendEvent")
