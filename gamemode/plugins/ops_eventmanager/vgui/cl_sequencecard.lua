...
function PANEL:SetSequence(key, data)
	...
	self.main:SetTooltip("Version hash: "..(mrp.Ops.EventManager.Sequences[key].VersionHash or "outdated"))
	...
	function newSeq:DoClick()
		local id = table.insert(mrp.Ops.EventManager.Sequences[key].Events, {
			Type = "empty",
			Prop = {},
			UID = nil,
			Delay = 0
		...
		table.Merge(mrp.Ops.EventManager.Sequences[key].Events[id].Prop, mrp.Ops.EventManager.Config.Events["empty"].Prop)

		panel:AddEvent(id, mrp.Ops.EventManager.Sequences[key].Events[id])
	end
	...
	function remSeq:DoClick()
		mrp.Ops.EventManager.Sequences[key] = nil
	...
	function saveSeq:DoClick()
		if not mrp.Ops.EventManager.Sequences[key].FileName then
			Derma_StringRequest("mrp", "Enter sequence file name:", nil, function(name)
				mrp.Ops.EventManager.Sequences[key].FileName = name
				mrp.Ops.EventManager.SequenceSave(key)
	...
	function uploadSeq:DoClick()
		mrp.Ops.EventManager.SequencePush(key)
	end
	...
	function uploadSeq:DoRightClick()
		...
		timer.Simple(0.1, function()
			net.Start("mrpOpsEMPlaySequence")
			net.WriteString(key)
			net.SendToServer()
		end)
...
function PANEL:AddEvent(id, eventdata)
	...
	function event:Paint(w, h)
		...
		local colTag = mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag
		if colTag and mrp.Ops.EventManager.Config.TagColours[colTag] then
			draw.RoundedBox(0, 0, 0, w, h, ColorAlpha(mrp.Ops.EventManager.Config.TagColours[colTag], 40))
		end

		draw.RoundedBox(0, 0, h-2, w, 2, Color(100, 100, 100, 150))

		local curEvents = mrp.Ops.EventManager.GetCurEvents()
		...
		if not curEvents[id + 1] and (mrp.Ops.EventManager.GetSequence() and panel.Sequence == mrp.Ops.EventManager.GetSequence()) then
			mrp_OpsEM_CurEvents[id] = CurTime()
		end
	...
	event.etypeicon:SetImage(mrp.Ops.EventManager.Config.CategoryIcons[mrp.Ops.EventManager.Config.Events[eventdata.Type].Cat])
	...
	function event.edelay:OnValueChanged(new)
		if tonumber(new) then
			...
			mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Delay = realNew
		end
	end
	...
	function event.eremove:DoClick()
		table.remove(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, id)
		panel.Dad:ReloadSequences()
	end
	...
	function event.etypebtn:DoClick()
		...
		for v,k in pairs(mrp.Ops.EventManager.Config.Events) do
			...
			if not cats[k.Cat] then
				...
				p:SetIcon(mrp.Ops.EventManager.Config.CategoryIcons[k.Cat])
				cats[k.Cat] = c
			end

			local parent = cats[k.Cat]
			parent:AddOption(v, function()
				mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Type = v
				mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop = {}
				table.Merge(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop, mrp.Ops.EventManager.Config.Events[v].Prop)
				panel.Dad:ReloadSequences()
			end)
		end
	...
	if table.Count(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop) == 0 then
	...
	function event.eprop:DoClick()
		...
		panel.Dad.Properties = vgui.Create("mrpPropertyEditor")
		panel.Dad.Properties:SetTable(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop, function(key, val)
			mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Prop[key] = val
		end)
		panel.Dad.Properties:SetTitle(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].Type.."("..id..") properties")
		...
		for v,k in pairs(mrp.Ops.EventManager.Config.TagColours) do
			...
			function a:DoClick()
				mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag = v
			end
		end
		...
		function noCol:DoClick()
			mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaTag = nil
		end
		...
		notes:SetValue(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaNotes or "")

		function notes:OnValueChange()
			mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].z_MetaNotes = self:GetValue()
		end
	end
	...
	event.euid:SetText(mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID or "")
	...
	function event.euid:OnValueChange(new)
		...
		if new == "" then
			mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID = nil
			return
		end

		mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id].UID = new
	end

	function event.euid:PaintOver(w, h)
		if mrp.Ops.EventManager.Config.Events[eventdata.Type].NeedUID and not self:IsEditing() and string.Trim(self:GetText(), " ") == "" then
	...
	function event.euid:GetAutoComplete(text)
		...
		for v,k in pairs(mrp.Ops.EventManager.Sequences[panel.Sequence].Events) do
			if v == id then
				continue
			end
	...
	local function moveToCustomSlot()
		Derma_StringRequest("mrp ops", "Enter where you wish to move this event to:", "", function(slot)
			if not tonumber(slot) then
				return
			end

			local me = mrp.Ops.EventManager.Sequences[panel.Sequence].Events[id]
			local oldId = id
			local size = table.Count(mrp.Ops.EventManager.Sequences[panel.Sequence].Events)
			slot = math.Clamp(math.floor(slot), 1, size)

			table.remove(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, id)
			table.insert(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, slot, me)
	...
	function event.emup:DoClick()
		...
		table.remove(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, id)
		table.insert(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, math.Clamp(oldId - 1, 1, size), me)
	...
	function event.edupe:DoClick()
		...
		table.insert(mrp.Ops.EventManager.Sequences[panel.Sequence].Events, math.Clamp(oldId + 1, 1, size), copy1(me))
...
vgui.Register("mrpSequenceCard", PANEL, "DPanel")