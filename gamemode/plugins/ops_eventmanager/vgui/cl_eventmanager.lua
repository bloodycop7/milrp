...
function PANEL:Init()
	...
	menuFile:AddOption("New sequence", function()
		Derma_StringRequest("mrp ops",
			"Enter sequence name:",
			nil, 
			function(text)
				mrp.Ops.EventManager.Sequences[text] = {
					Name = text, 
					FileName = text,
					Events = {}, 
					VersionHash = mrp.Ops.EventManager.GetVersionHash()
				}
	...
	menuFile:AddOption("Load sequence", function()
		...
		browser:SetBaseFolder("mrp/ops/eventmanager")
		browser:SetOpen(true)
		browser:SetCurrentFolder("mrp/ops/eventmanager")
		browser:SetName("eventmanager")
		browser:SetFileTypes("*.json")

		function browser:OnSelect(path)
			local result, msg = mrp.Ops.EventManager.SequenceLoad(path)
...
function PANEL:ReloadSequences()
	...
	for v,k in pairs(mrp.Ops.EventManager.Sequences) do
		local seqPanel = self.seqScroll:Add("mrpSequenceCard")
...
function PANEL:SetupPlayer(sequences)
	...
	for v,k in pairs(sequences) do
		...
		function sequence:Paint(w, h)
			...
			if mrp.Ops.EventManager.GetSequence() and mrp.Ops.EventManager.GetSequence() == k then
		...
		function stop:DoClick()
			net.Start("mrpOpsEMStopSequence")
			net.WriteString(k)
			net.SendToServer()
		end
	end
...
vgui.Register("mrpEventManager", PANEL, "DFrame")