...
function meta:SetTeam(teamID, forced)
	...
	if teamData.model then
        if ( isstring(mrp.Teams.Stored[self:Team()].model) ) then
            self:SetModel(mrp.Teams.Stored[self:Team()].model)
        elseif ( istable(mrp.Teams.Stored[self:Team()].model) ) then
            self:SetModel(table.Random(mrp.Teams.Stored[self:Team()].model))
        end
	else
