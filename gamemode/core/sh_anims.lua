...
-- this animation system is HEAVILY based on nutscripts system with partial recodes or changes to suit mrp.
-- i claim no credit for this
-- check out NS at https://github.com/rebel1324/NutScript/

mrp.Anim = mrp.Anim or {}
mrp.Anim.citizen_male = {
	normal = {
...
mrp.Anim.citizen_female = {
	...
	glide = ACT_GLIDE,
	vehicle = mrp.Anim.citizen_male.vehicle
}
mrp.Anim.metrocop = {
	normal = {
...
mrp.Anim.zombie.melee = mrp.Anim.zombie.normal
...
function mrp.Anim.SetModelClass(model, class)
	if not mrp.Anim[class] then
...
function mrp.Anim.GetModelClass(model)
...
mrp.Anim.SetModelClass("models/bread/cod/characters/milsim/shadow_company.mdl", "player")

--mrp.Anim.SetModelClass("models/breen.mdl", "player")
--mrp.Anim.SetModelClass("models/nicrobe/gallahan.mdl", "player")
...
do
	...
	function meta:ForceSequence(sequence, callback, time, noFreeze)
		...
		if sequence and sequence > 0 then
			...
			self.mrpSeqCallback = callback
			self.mrpForceSeq = sequence
			...
			if time > 0 then
				timer.Create("mrpSeq"..self:EntIndex(), time, 1, function()
	...
	function meta:leaveSequence()
		...
		self.mrpForceSeq = nil

		if self.mrpSeqCallback then
			self:mrpSeqCallback()
		end
	end
	...
	if SERVER then
		util.AddNetworkString("mrpSeqSet")
	...
	if CLIENT then
		net.Receive("mrpSeqSet", function()
...
local getModelClass = mrp.Anim.GetModelClass
...
function GM:TranslateActivity(ply, act)
	...
	if class == "player" then
		if IsValid(weapon) and not ply.IsWeaponRaised(ply) and ply.OnGround(ply) then
			...
			local animTree = mrp.Anim.player[holdType]
...
function GM:CalcMainActivity(ply, velocity)
	...
	return seqIdeal, ply.mrpForceSeq or oldSeqOverride or ply.CalcSeqOverride
end

function GM:DoAnimationEvent(ply, event, data)
	local model = ply:GetModel():lower()
	local class = mrp.Anim.GetModelClass(model)
	...
	if class == "player" then
		...
		if IsValid(weapon) then
			...
			local animation = mrp.Anim[class][holdType]
