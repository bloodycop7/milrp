...
function PANEL:AddText(...)
	local text = "<font=".."mrp-Font23>"
	local plainText = ""
	local luaMsg = {}

	if mrp.customChatFont then
		text = "<font="..mrp.customChatFont..">"
	...
	if mrp.customChatPlayer then
		textElement.player = mrp.customChatPlayer
	end

	mrp.customChatPlayer = nil
	...
	textElement.OnMousePressed = function()
		...
		if LocalPlayer():IsAdmin() and IsValid(textElement.player) then
			local banPly = subMenu:AddOption("OOC timeout sender", function()
				Derma_StringRequest("mrp", "Enter timeout length in minutes:", "15", function(x)
...
vgui.Register("mrpRichText", PANEL, "DScrollPanel")