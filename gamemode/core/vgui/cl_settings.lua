...
function PANEL:Init()
	...
	for v,k in pairs(mrp.Settings) do
		...
		if settingType == "tickbox" then
			...
			tickbox:SetValue(mrp.GetSetting(v))

			function tickbox:OnChange(value)
				mrp.SetSetting(v, value)

				if k.needsRestart then
					Derma_Message("You may need to reconnect to the server to fully activate this setting.", "mrp", "Ok")
				end
			end
		elseif settingType == "plainint" then
			...
			numberEntry:SetValue(mrp.GetSetting(v))
		...
		elseif settingType == "slider" then
			...
			numSlider:SetValue(mrp.GetSetting(v))
		...
		elseif settingType == "dropdown" then
			...
			dropdown:SetValue(mrp.GetSetting(v))
	...
	function button:DoClick()
		LocalPlayer():ConCommand("mrp_resetsettings")
		panelSelf:Remove()
    end

end

vgui.Register("mrpSettings", PANEL, "DFrame")