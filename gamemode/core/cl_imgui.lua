...
function mrp.imgui.IsDeveloperMode()
	return not mrp.imgui.DisableDeveloperMode and devCvar:GetInt() > 0
end

local _devMode = false -- cached local variable updated once in a while

function mrp.imgui.Hook(name, id, callback)
...
mrp.imgui.Hook("PreRender", "Input", function()
...
function mrp.imgui.Start3D2D(pos, angles, scale, distanceHide, distanceFadeStart)
	...
	_devMode = mrp.imgui.IsDeveloperMode()
...
function mrp.imgui.Entity3D2D(ent, lpos, lang, scale, ...)
	gState.entity = ent
	local ret = mrp.imgui.Start3D2D(ent:LocalToWorld(lpos), ent:LocalToWorldAngles(lang), scale, ...)
	if not ret then
		gState.entity = nil
	end
	return ret
...
function mrp.imgui.ExpandRenderBoundsFromRect(x, y, w, h)
...
function mrp.imgui.CursorPos()
	local mx, my = gState.mx, gState.my
	return mx, my
end

function mrp.imgui.IsHovering(x, y, w, h)
	local mx, my = gState.mx, gState.my
	return mx and my and mx >= x and mx <= (x + w) and my >= y and my <= (y + h)
end
function mrp.imgui.IsPressing()
	return shouldAcceptInput() and gState.pressing
end
function mrp.imgui.IsPressed()
...
function mrp.imgui.xFont(font, defaultSize)
	-- special font
...
function mrp.imgui.xButton(x, y, w, h, borderWidth, borderClr, hoverClr, pressColor)
	local bw = borderWidth or 1

	local bgColor = mrp.imgui.IsHovering(x, y, w, h) and mrp.imgui.skin.backgroundHover or mrp.imgui.skin.background
	local borderColor =
		((mrp.imgui.IsPressing() and mrp.imgui.IsHovering(x, y, w, h)) and (pressColor or mrp.imgui.skin.borderPress))
		or (mrp.imgui.IsHovering(x, y, w, h) and (hoverClr or mrp.imgui.skin.borderHover))
		or (borderClr or mrp.imgui.skin.border)
	...
	return shouldAcceptInput() and mrp.imgui.IsHovering(x, y, w, h) and gState.pressed
end

function mrp.imgui.xCursor(x, y, w, h)
	local fgColor = mrp.imgui.IsPressing() and mrp.imgui.skin.foregroundPress or mrp.imgui.skin.foreground
	local mx, my = gState.mx, gState.my
...
function mrp.imgui.xTextButton(text, font, x, y, w, h, borderWidth, color, hoverClr, pressColor)
	local fgColor =
		((mrp.imgui.IsPressing() and mrp.imgui.IsHovering(x, y, w, h)) and (pressColor or mrp.imgui.skin.foregroundPress))
		or (mrp.imgui.IsHovering(x, y, w, h) and (hoverClr or mrp.imgui.skin.foregroundHover))
		or (color or mrp.imgui.skin.foreground)

	local clicked = mrp.imgui.xButton(x, y, w, h, borderWidth, color, hoverClr, pressColor)

	font = mrp.imgui.xFont(font, math.floor(h * 0.618))
