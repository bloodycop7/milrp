...
if CLIENT then
	...
	local function PaintSeatSwitcher( ent, X, Y )
		...
		for _, Pod in pairs( pSeats ) do
			...
			if I >= 0 then
				if I == MySeat then
					mrp.DrawBox({
                        x = X + Offset - HiderOffset, 
                        y = yPos + I * 30, 
                        w = 35 + HiderOffset, 
                        h = 25,
                        col = Color(0, 162, 255, 200 + 50 * smHider)
                    })
				else
                    mrp.DrawBox({
                        x = X + Offset - HiderOffset, 
                        ...
                        w = 35 + HiderOffset, 
                        h = 25,
                        col = Color(0, 0, 0, 100 + 50 * smHider)
                    })
				end
				if I == SeatCount then
				...
				if Hide then
					if Passengers[I] then
						draw.DrawText( Passengers[I], "mrp-Font18", X + 40 + Offset - HiderOffset, yPos + I * 30 + 2.5, Color( 255, 255, 255,  Alpha1 ), TEXT_ALIGN_LEFT )
					else
						draw.DrawText( "-", "mrp-Font16", X + 40 + Offset - HiderOffset, yPos + I * 30 + 2.5, Color( 255, 255, 255,  Alpha1 ), TEXT_ALIGN_LEFT )
					end
					
					draw.DrawText( "["..I.."]", "mrp-Font16", X + 17 + Offset - HiderOffset, yPos + I * 30 + 2.5, Color( 255, 255, 255, Alpha1 ), TEXT_ALIGN_CENTER )
				else
					if Passengers[I] then
						draw.DrawText( "[^"..I.."]", "mrp-Font18", X + 17 + Offset - HiderOffset, yPos + I * 30 + 2.5, Color( 255, 255, 255, Alpha1 ), TEXT_ALIGN_CENTER )
					else
						draw.DrawText( "["..I.."]", "mrp-Font16", X + 17 + Offset - HiderOffset, yPos + I * 30 + 2.5, Color( 255, 255, 255, Alpha1 ), TEXT_ALIGN_CENTER )
					end
				end
	...
	--[[hook.Add("HUDPaint", "!!!!!LFS_hud", function()
		local ply = LocalPlayer()
			...
			local Throttle = Parent:GetThrottlePercent()

			local speed = math.Round(vel * 0.09144,0)

			local ZPos = math.Round( Parent:GetPos().z,0)
			if (ZPos + simfphys.LFS.AltitudeMinZ)< 0 then simfphys.LFS.AltitudeMinZ = math.abs(ZPos) end
			local alt = math.Round( (Parent:GetPos().z + simfphys.LFS.AltitudeMinZ) * 0.0254,0)

			local AmmoPrimary = Parent:GetAmmoPrimary()
			local AmmoSecondary = Parent:GetAmmoSecondary()

			draw.DrawText("THR - "..Throttle, "mrp-Font22", 15, 30, color_white, TEXT_ALIGN_LEFT)
			draw.DrawText("SPEED - "..speed, "mrp-Font22", 15, 60, color_white, TEXT_ALIGN_LEFT)
			draw.DrawText("ALT - "..alt, "mrp-Font22", 15, 90, color_white, TEXT_ALIGN_LEFT)

			if ( AmmoPrimary != -1 and AmmoSecondary != -1 ) then
				draw.DrawText("PRIMARY AMMO - "..AmmoPrimary, "mrp-Font22", 15, 120, color_white, TEXT_ALIGN_LEFT)
				draw.DrawText("SECONDARY AMMO - "..AmmoSecondary, "mrp-Font22", 15, 150, color_white, TEXT_ALIGN_LEFT)
			end
